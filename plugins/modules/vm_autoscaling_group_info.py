...
DBAAS_POSTGRES_USER_AGENT = 'ansible-module/%s_ionos-cloud-sdk-python/%s' % ( __version__, ionoscloud_vm_autoscaling.__version__)
...
requirements:
    - "python >= 2.6"
    - "ionoscloud_vm_autoscaling >= 1.0.0"
author:
...
def main():
    ...
    if not HAS_SDK:
        module.fail_json(msg='ionoscloud_vm_autoscaling is required for this module, run `pip install ionoscloud_vm_autoscaling`')

    vm_autoscaling_api_client = ionoscloud_vm_autoscaling.ApiClient(get_sdk_config(module, ionoscloud_vm_autoscaling))
    vm_autoscaling_api_client.user_agent = DBAAS_POSTGRES_USER_AGENT

    check_required_arguments(module, OBJECT_NAME)
    try:
        results = []
        for cluster in ionoscloud_vm_autoscaling.GroupsApi(vm_autoscaling_api_client).autoscaling_groups_get(depth=3).items:
